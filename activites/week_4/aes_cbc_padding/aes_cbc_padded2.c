...
void pkcs7_pad(unsigned char *plaintext, int plaintext_len, int block_size, unsigned char *padded_plaintext, int *padded_len) {
...
int main(void) {
    ...
    pkcs7_pad(plaintext, plaintext_len, 16, padded_plaintext, &padded_len);
    ...
    if (!ctx) handleErrors();
    EVP_CIPHER_CTX_set_padding(ctx, 0); // turn off the padding in the EVP function
    if (1 != EVP_EncryptInit_ex(ctx, EVP_aes_128_cbc(), NULL, key, iv))
        handleErrors();

    if (1 != EVP_EncryptUpdate(ctx, ciphertext, &ciphertext_len, padded_plaintext, padded_len))
        handleErrors();
